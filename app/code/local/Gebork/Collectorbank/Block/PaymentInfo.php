...
class Gebork_Collectorbank_Block_PaymentInfo extends Mage_Payment_Block_Info
{
    ...
    public function getLogoUrl() {
        return $this->getSkinUrl('images' . DS . 'gebork' . DS . 'collectorbank' . DS . 'logos' . DS . $this->getMethod()->getLogo());
    }
            ...
            if ($this->getIsSecureMode()) {
                if ($this->isInvoice() AND $this->getInvoiceType() != 3) {
                    ...
                    if ($invoice->getId() AND $invoice->getTransactionId()) {
                        $collectorData = Mage::getModel('collectorbank/informationservice_getinvoice')
                            ->setRequest($payment, array('invoice' => $invoice))
                            ->getInvoice();
