...
class Gebork_Collectorbank_Model_Invoiceservice_Abstract extends Mage_Core_Model_Abstract
{
	...
	const SAP_WSDL_URL_PROD  = "https://ecommercetest.collector.se/v3.0/InvoiceServiceV33.svc?singleWsdl";
	const SAP_WSDL_URL_TEST  = "https://ecommercetest.collector.se/v3.0/InvoiceServiceV33.svc?singleWsdl";
    ...
    public function createSoapClient($trace = false, $headers = array()) {
		...
		//echo $wsdlPath . DS . self::WSDL_FILE_PROD, array('trace' => $trace);
		
		
        //if ($this->helper->getModuleConfig('settings/sandbox_mode', $this->getStoreId())) {
			
          // $client = new SoapClient($wsdlPath . DS . self::WSDL_FILE_TEST, array('trace' => $trace));
			  $client = new SoapClient(self::SAP_WSDL_URL_TEST, array('trace' => $trace));
           $client->__setLocation('https://ecommercetest.collector.se/v3.0/InvoiceServiceV33.svc');
       ...
       // $headers['Username'] = $this->helper->getModuleConfig('settings/api_username', $this->getStoreId());
        //$headers['Password'] = $this->helper->getModuleConfig('settings/api_password', $this->getStoreId());
