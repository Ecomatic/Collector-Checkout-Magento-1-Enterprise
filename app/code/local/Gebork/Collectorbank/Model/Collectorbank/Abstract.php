<?php
class Gebork_Collectorbank_Model_Collectorbank_Abstract extends Mage_Payment_Model_Method_Abstract
{
    ...
    public function getLogo() {
        return $this->getConfigData('collector_logo');
    }
    ...
    public function isAvailable($quote = null) {
        ...
        $creditAmount = 0;
        if ($quote->getData('base_gift_cards_amount') > 0) {
            $creditAmount += $quote->getData('base_gift_cards_amount');
        }
        if ($quote->getData('base_customer_balance_amount') > 0) {
            $creditAmount += $quote->getData('base_customer_balance_amount');
        }
        if ($quote->getData('base_reward_currency_amount') > 0) {
            $creditAmount += $quote->getData('base_reward_currency_amount');
        }
        if (!$this->getConfigData('allow_credit') AND $creditAmount > 0) {
            return false;
        }
        return true;
    }
